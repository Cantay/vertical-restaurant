...
class PaymentProvider(models.Model):
    ...
    min_amount_for_installment = fields.Monetary(string='Taksit İçin Min. Tutar', default=100.0, currency_field='main_currency_id')
    ...
    total_volume = fields.Monetary(string='Toplam Hacim', compute='_compute_statistics', currency_field='main_currency_id')
    success_rate = fields.Float(string='Başarı Oranı (%)', compute='_compute_statistics')
    transaction_ids = fields.One2many('payment.transaction', 'provider_id', string='İşlemler')
    ...
    def _est_refund(self, transaction, amount):
        """EST POS için iade"""
        xml_data = f"""<?xml version="1.0" encoding="ISO-8859-9"?>
        <CC5Request>
            <Name>{self.api_username}</Name>
            <Password>{self.api_password}</Password>
            <ClientId>{self.api_client_id}</ClientId>
            <Type>Credit</Type>
            <OrderId>{transaction.vpos_order_id}</OrderId>
            <Total>{amount}</Total>
        </CC5Request>"""
        ...
        return self._parse_est_response(response.text)
    ...
    def _garanti_refund(self, transaction, amount):
        ...
        hash_data = f"{self.api_terminal_id}{transaction.vpos_order_id}{str(int(amount * 100))}{security_data}"
        hash_value = self._generate_hash(hash_data, 'sha256').upper()
        
        xml_data = f"""<?xml version="1.0" encoding="UTF-8"?>
        <GVPSRequest>
            <Mode>{'PROD' if self.environment == 'production' else 'TEST'}</Mode>
            <Version>0.01</Version>
            <Terminal>
                <ProvUserID>{self.api_provision_user}</ProvUserID>
                <HashData>{hash_value}</HashData>
                <UserID>{self.api_username}</UserID>
                <ID>{self.api_terminal_id}</ID>
                <MerchantID>{self.api_merchant_id}</MerchantID>
            </Terminal>
            <Customer>
                <IPAddress>{transaction.partner_ip_address or '127.0.0.1'}</IPAddress>
                <EmailAddress>{transaction.partner_email or ''}</EmailAddress>
            </Customer>
            <Order>
                <OrderID>{transaction.vpos_order_id}</OrderID>
                <GroupID></GroupID>
            ...
            <Transaction>
                ...
                <CurrencyCode>{self._get_currency_code(transaction.currency_id)}</CurrencyCode>
            </Transaction>
        </GVPSRequest>"""
        ...
        return self._parse_garanti_response(response.text)
