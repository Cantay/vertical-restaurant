...
class PaymentTransaction(models.Model):
    ...
    # POS İşlem Bilgileri
    vpos_order_id = fields.Char(string='POS Sipariş ID', index=True)
    ...
    installment_amount = fields.Monetary(string='Taksitli Tutar')
    commission_amount = fields.Monetary(string='Komisyon Tutarı')
    
    # İade/İptal Bilgileri
    is_refunded = fields.Boolean(string='İade Edildi', default=False)
    refund_amount = fields.Monetary(string='İade Tutarı', default=0.0)
    ...
    def _send_payment_request(self):
        ...
        try:
            ...
            self.vpos_order_id = f"ODOO_{self.id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def _create_refund_move(self, amount):
        ...
        self.ensure_one()
        # Muhasebe kaydı oluşturma mantığı
        pass
