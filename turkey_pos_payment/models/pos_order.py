...
class PosOrder(models.Model):
    ...
    name = fields.Char(string='Sipariş No', required=True, index=True, copy=False,
                       default=lambda self: self.env['ir.sequence'].next_by_code('vpos.order'))
    ...
    # Tutar Bilgileri
    amount = fields.Monetary(related='transaction_id.amount', string='Tutar', store=True)
    currency_id = fields.Many2one(related='transaction_id.currency_id', string='Para Birimi', store=True)
    
    # POS Bilgileri
    vpos_order_id = fields.Char(related='transaction_id.vpos_order_id', string='POS Sipariş ID', store=True)
    ...
    # İade Bilgileri
    refund_amount = fields.Monetary(related='transaction_id.refund_amount', string='İade Tutarı', store=True)
...
class PosOrderLine(models.Model):
    _name = 'vpos.order.line'
    _description = 'POS Sipariş Satırı'

    order_id = fields.Many2one('vpos.order', string='Sipariş', required=True, ondelete='cascade')
    ...
    price_unit = fields.Monetary(string='Birim Fiyat')
    discount = fields.Float(string='İndirim (%)', default=0.0)
    
    # Tutarlar
    price_subtotal = fields.Monetary(string='Ara Toplam', compute='_compute_amounts', store=True)
    price_total = fields.Monetary(string='Toplam', compute='_compute_amounts', store=True)
...
class PosReconciliation(models.Model):
    ...
    total_amount = fields.Monetary(string='Toplam Tutar', compute='_compute_statistics')
    total_commission = fields.Monetary(string='Toplam Komisyon', compute='_compute_statistics')
    net_amount = fields.Monetary(string='Net Tutar', compute='_compute_statistics')
    
    # Para Birimi
    currency_id = fields.Many2one(related='provider_id.main_currency_id', string='Para Birimi')
    
    # İlişkiler
    order_ids = fields.Many2many('vpos.order', string='Siparişler')
    ...
    def action_load_transactions(self):
        ...
        for rec in self:
            ...
            orders = self.env['vpos.order'].search([
