...
class TurkeyPosController(http.Controller):
    ...
    @http.route('/payment/turkey_pos/3d_return/<string:provider_code>', 
                type='http', auth='public', csrf=False, methods=['POST', 'GET'])
    def turkey_pos_3d_return(self, provider_code, **post):
        """3D Secure dönüşünü işler"""
        _logger.info('3D Return received for provider: %s, data: %s', provider_code, post)
        ...
        try:
            ...
            if not provider:
                _logger.error('Provider not found: %s', provider_code)
                return request.redirect('/payment/error')
            
            # İşlem referansını bul
            order_id = post.get('oid') or post.get('orderId') or post.get('OrderId')
            if not order_id:
            ...
            transaction = request.env['payment.transaction'].sudo().search([
                ('vpos_order_id', 'like', order_id.split('_')[0] if '_' in order_id else order_id)
            ], limit=1)
            ...
            # 3D dönüşünü işle
            result = provider.process_3d_return(post)
            
            if result['success']:
                transaction._process_notification_data(post)
                return request.redirect('/payment/confirmation')
            else:
                transaction.write({
                    'state': 'error',
                    'error_message': result.get('message', _('3D authentication failed')),
                })
                return request.redirect('/payment/error')
